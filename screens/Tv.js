...
const Tv = ({ navigation, route }) => {
  ...
  const fetchTv = async () => {
    const res = await fetch(
      `${TMDB_API_URI}/discover/tv?api_key=${TMDB_API_KEY}&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&with_genres=${genresToURL}`
    );

    const json = await res.json();
    setTv(tv.concat(json.results));
  };
